...
const DeptBar = ({ present, onLeave, absent, wfh }) => {
  ...
  return (
    <div className="flex w-full h-2 rounded-full overflow-hidden gap-px">
      ...
      <div className="bg-cyan-400 transition-all"    style={{ width: `${((wfh ?? 0) / total) * 100}%` }} />
      <div className="bg-amber-400 transition-all"   style={{ width: `${(onLeave / total) * 100}%` }} />
...
const AdminDashboardUI = ({ data, isLoading, isError }) => {
  ...
  const totalRequests = data?.leaveBreakdown?.reduce((a, b) => a + b.count, 0) || 1
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Admin Leave Dashboard</h1>
          <p className="text-muted-foreground text-sm mt-0.5">
            {isLoading ? "Loading…" : `${data?.month} ${data?.year} — ${data?.totalEmployees} employees`}
          </p>
        </div>
        <Badge variant="outline" className="text-xs px-3 py-1">🔴 Live</Badge>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <MetricCard icon="🏖️" label="Leaves This Month"  value={data?.thisMonth?.totalLeaves}        sub="full-day leaves"             accent="bg-blue-500/10"    />
          <MetricCard icon="❌" label="Absences"            value={data?.thisMonth?.totalAbsent}        sub="unauthorized & unplanned"   accent="bg-red-500/10"     />
          <MetricCard icon="✅" label="Avg. Daily Present"  value={data?.thisMonth?.avgDailyPresent}    sub={`of ${data?.totalEmployees}`} accent="bg-emerald-500/10" />
          <MetricCard icon="⏳" label="Pending Approvals"  value={data?.pendingApprovals?.length}      sub="awaiting your action"       accent="bg-amber-500/10"   />
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 gap-3">
          <MetricCard icon="🏠" label="Work From Home (Month)" value={data?.thisMonth?.totalWFH}     sub="approved WFH days"   accent="bg-cyan-500/10"   />
          <MetricCard icon="🌗" label="Half Days (Month)"      value={data?.thisMonth?.totalHalfDay} sub="morning & afternoon" accent="bg-yellow-500/10" />
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div key={item.type}>
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          <div className={`w-2.5 h-2.5 rounded-full ${LEAVE_COLOR[item.color] ?? "bg-gray-400"}`} />
        ...
        <Card className="shadow-sm lg:col-span-2">
          ...
          <CardContent>
            {isLoading
              ? Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-10 mb-2 rounded-lg" />)
              : (
                <div className="space-y-3">
                  {data?.departmentStats?.map((dept) => (
                    <div key={dept.department}>
                      <div className="flex items-center justify-between mb-1 text-sm">
                        <span className="font-medium w-28 truncate">{dept.department}</span>
                        <div className="flex gap-2 text-xs text-muted-foreground">
                          <span className="text-emerald-600 font-semibold">✓ {dept.present}</span>
                          <span className="text-cyan-600 font-semibold">🏠 {dept.wfh ?? 0}</span>
                          <span className="text-amber-600 font-semibold">⏳ {dept.onLeave}</span>
                          <span className="text-red-500 font-semibold">✗ {dept.absent}</span>
                        </div>
                      </div>
                      <DeptBar present={dept.present} onLeave={dept.onLeave} absent={dept.absent} wfh={dept.wfh} />
                    </div>
                  ))}
      ...
      <Card className="shadow-sm">
        <CardHeader className="pb-2">
          <CardTitle className="text-base flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground font-normal">by total days away</span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading
            ? Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-12 mb-2 rounded-lg" />)
            : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <CardContent className="space-y-2">
            {isLoading
              ...
              : data?.pendingApprovals?.slice(0, 5).map((item) => (
                  <div key={item.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/40 border border-border/50 gap-2">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold leading-none truncate">{item.name}</p>
                      <p className="text-xs text-muted-foreground mt-1">{item.type} · {item.from} → {item.to}</p>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <Badge variant="outline" className={`text-[10px] px-1.5 ${SUBTYPE_BADGE[item.subType] ?? ""}`}>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent>
            {isLoading
              ? Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-10 mb-2 rounded-lg" />)
              : (
                <div className="space-y-1">
                  {data?.recentActivity?.map((item, idx) => (
                    <div key={item.id}>
                      <div className="flex items-center justify-between py-2 text-sm">
                        <div className="flex items-center gap-2 min-w-0">
                          <div className={`w-2 h-2 rounded-full shrink-0 ${
                            ...
                            : item.action.includes("Absent") ? "bg-red-400"
                            : "bg-blue-500"
                          }`} />
                        ...
                        <div className="flex items-center gap-2 shrink-0 ml-2">
                          <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${
                            item.action.includes("Approved") ? STATUS_STYLE.approved
                            : item.action.includes("Rejected") ? STATUS_STYLE.rejected
                      ...
                      {idx < (data?.recentActivity?.length ?? 0) - 1 && <Separator />}
                    </div>
                  ))}
                </div>
              )}
...
AdminDashboardUI.propTypes = {
  data: PropTypes.shape({
    ...
    leaveBreakdown: PropTypes.arrayOf(
      PropTypes.shape({ type: PropTypes.string, count: PropTypes.number, color: PropTypes.string })
    ),
